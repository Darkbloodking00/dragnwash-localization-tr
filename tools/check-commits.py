...
# message. A documentation address such as docs.anthropic.com is left alone, so a
# commit may still cite a page it followed. The @anthropic.com rule needs the at
# sign, so it catches the address in a trailer and not a link to a page.
FORBIDDEN_MESSAGE = [
...
#
# Addresses are matched whole, names only where no person would use them. That
# matters here: people from a dozen language communities commit to this
# repository, Claude is an ordinary French given name, and a translator called
# Claude committing from their own address must pass. So the name rules ask for
# a model or a bot suffix after the name before they refuse anything.
#
